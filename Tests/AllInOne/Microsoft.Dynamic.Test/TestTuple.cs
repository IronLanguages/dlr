...
namespace Microsoft.Dynamic.Test {
    ...
    public class TestTuple {
        public void VerifyTuple(int size) {
            ...
            Assert.IsNotNull(mi, "Could not find Tuple.MakeTupleType");
            ...
            if (size <= MutableTuple.MaxSize) {
                ...
                if (tupleType.GetGenericArguments().Length <= size) {
                    ...
                    Assert.ThrowsException<ArgumentOutOfRangeException>(delegate () { t.SetValue(size, 3); });
                    Assert.ThrowsException<ArgumentOutOfRangeException>(delegate () { t.GetValue(size); });
                } else {
                    ...
                    Assert.ThrowsException<InvalidCastException>(delegate () { t.SetValue(size, 3); });
